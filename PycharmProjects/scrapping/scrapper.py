...
http_status_print = http.server.BaseHTTPRequestHandler.responses
...
def get_auth():
    ...
    try:
        filehandle = open(config_file, "r", encoding="utf-8")
...
def make_request(client, url, request_headers={}, error_string="Failed Request", method="GET", body=None):
    ...
    if resp.status >= 200 and resp.status < 300:
        ...
        if resp.status in status_codes:
            ...
            log_diagnostic_info(client, url, request_headers, method, body, resp, content,
                                http_status_print[resp.status][1])
...
if __name__ == "__main__":
    ...
    ##############################
    '''
    print("tous les employés d'une compagnie")
    response = make_request(client, "https://api.linkedin.com/v1/people-search:(people:(first-name,last-name))?keywords=sinay")
    #response = make_request(client, "http://api.linkedin.com/v1/people-search?company-name=sinay&current-company=true")
    ...
    api_url = "http://api.linkedin.com/v1/people/~/shares";
    response = make_request(client, api_url, {'Content-Type': 'application/json'}, "Failed to post share", "POST",
                            json_content)
    exit();
